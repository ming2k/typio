/**
 * @file log.h
 * @brief Logging utilities (public ABI).
 *
 * `typio_log` is a header-only static inline variadic wrapper that
 * forwards a formatted string to the Rust-side `_typio_log` symbol.
 */

#ifndef TYPIO_LOG_H
#define TYPIO_LOG_H

#include "typio/types.h"
#include <stdarg.h>
#include <stdio.h>

#ifdef __cplusplus
extern "C" {
#endif

#define TYPIO_LOG_RECENT_CAPACITY 256

/* Sink implemented in Rust */
void _typio_log(TypioLogLevel level, const char *msg);

/* Level + callback control */
void typio_log_set_level(TypioLogLevel level);
TypioLogLevel typio_log_get_level(void);
void typio_log_set_callback(TypioLogCallback callback, void *user_data);
void typio_log_set_recent_dump_path(const char *path);
bool typio_log_dump_recent(const char *path);
bool typio_log_dump_recent_to_configured_path(void);

#if defined(__GNUC__) || defined(__clang__)
__attribute__((format(printf, 2, 3)))
#endif
static inline void typio_log(TypioLogLevel level, const char *format, ...) {
    va_list args;
    char buf[1024];
    va_start(args, format);
    vsnprintf(buf, sizeof(buf), format, args);
    va_end(args);
    _typio_log(level, buf);
}

#define typio_log_debug(...)   typio_log(TYPIO_LOG_DEBUG, __VA_ARGS__)
#define typio_log_info(...)    typio_log(TYPIO_LOG_INFO, __VA_ARGS__)
#define typio_log_warning(...) typio_log(TYPIO_LOG_WARNING, __VA_ARGS__)
#define typio_log_error(...)   typio_log(TYPIO_LOG_ERROR, __VA_ARGS__)

#ifdef __cplusplus
}
#endif

#endif /* TYPIO_LOG_H */
