...
pub struct TypioKeyboardEngineOps {
    ...
    // Optional session durability ops (Stage 3a). The Rust core only passes
    // the ops pointer through; these are dereferenced by the C frontend.
    // Mirrored here to keep the #[repr(C)] layout matching the C header.
    pub snapshot_session: Option<extern "C" fn(*mut TypioEngine, *mut TypioInputContext, *mut *mut c_char, *mut usize) -> TypioResult>,
    pub restore_session: Option<extern "C" fn(*mut TypioEngine, *mut TypioInputContext, *const c_char, usize) -> TypioResult>,
}
