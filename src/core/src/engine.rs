...
#[no_mangle]
pub extern "C" fn typio_engine_has_session_ops(engine: *const TypioEngine) -> bool {
    if engine.is_null() {
        return false;
    }
    unsafe {
        ...
        let keyboard = &*e.keyboard;
        keyboard.snapshot_session.is_some() && keyboard.restore_session.is_some()
    }
}

#[no_mangle]
pub extern "C" fn typio_engine_snapshot_session(
    engine: *mut TypioEngine,
    ctx: *mut TypioInputContext,
    ...
    unsafe {
        ...
        let keyboard = &*e.keyboard;
        match keyboard.snapshot_session {
            Some(snapshot) => snapshot(engine, ctx, out_data, out_size),
...
#[no_mangle]
pub extern "C" fn typio_engine_restore_session(
    engine: *mut TypioEngine,
    ctx: *mut TypioInputContext,
    data: *const c_char,
    size: usize,
    ...
    unsafe {
        ...
        let keyboard = &*e.keyboard;
        match keyboard.restore_session {
            Some(restore) => restore(engine, ctx, data, size),
            None => TypioResult::TypioErrorNotFound,
        }
    }
}
