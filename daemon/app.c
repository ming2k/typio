...
#endif
static void typio_daemon_update_ipc_bus_state(TypioDaemonApp *app);
...
static void typio_daemon_update_tray_tooltip(TypioDaemonApp *app) {
    ...
    if (app->state_controller) {
        ...
        TypioEngineManager *manager =
            typio_instance_get_engine_manager(app->instance);
        ...
        keyboard_label =
            keyboard ? typio_engine_label_from_info(keyboard->info) : nullptr;
        voice_label =
            voice ? typio_engine_label_from_info(voice->info) : nullptr;
    }
...
#endif
    typio_daemon_update_ipc_bus_state(app);
#ifdef HAVE_STATUS_BUS
...
static void typio_daemon_print_startup_banner(TypioDaemonApp *app) {
    ...
    typio_log_info("Starting %s", typio_build_display_string());
    ...
    if (active_keyboard) {
        typio_log_info("Active keyboard engine: %s", typio_engine_get_name(active_keyboard));
    } else {
...
static void typio_daemon_update_tray_engine_status(TypioDaemonApp *app) {
    ...
    if (app->state_controller) {
        ...
        if (!icon_name || !*icon_name) {
            icon_name = (active_keyboard && active_keyboard->info &&
                         active_keyboard->info->icon)
                            ? active_keyboard->info->icon
                            : "typio-keyboard";
        }
        is_active = active_keyboard != nullptr;
    }
...
static void typio_daemon_update_ipc_bus_state(TypioDaemonApp *app) {
    if (app && app->ipc_bus) {
        typio_ipc_bus_emit_properties_changed(app->ipc_bus);
    }
}
...
static void typio_daemon_on_state_changed(void *user_data,
...
static void typio_daemon_init_ipc_bus(TypioDaemonApp *app) {
...
static void typio_daemon_init_status_bus(TypioDaemonApp *app) {
    ...
    if (app->status_bus) {
        typio_status_bus_set_stop_callback(app->status_bus,
                                           typio_daemon_request_stop,
                                           app);
        typio_log_info("D-Bus status interface initialized");
    } else {
        typio_log_warning("D-Bus status interface not available");
    }
#else
    (void) app;
#endif
    ...
    if (app->state_controller) {
        typio_state_controller_add_listener(
            ...
            (TypioStateListener){ .user_data = app,
                                  .callback = typio_daemon_on_state_changed });
        typio_state_controller_sync(app->state_controller);
    }
