...
struct TypioVoiceService {
    ...
    /* Idle timeout: unload model after N ms of inactivity */
    int idle_timer_fd;
    int idle_shutdown_efd;
...
static void *voice_idle_thread(void *arg) {
    TypioVoiceService *svc = arg;
    struct pollfd fds[2] = {
        { .fd = svc->idle_timer_fd, .events = POLLIN },
        { .fd = svc->idle_shutdown_efd, .events = POLLIN },
    };
    ...
    while (1) {
        ...
        uint64_t exp;
        if (read(svc->idle_timer_fd, &exp, sizeof(exp)) != sizeof(exp))
            continue;

        pthread_mutex_lock(&svc->buffer_mutex);
        if (svc->idle_armed && svc->state == TYPIO_VOICE_IDLE &&
            svc->voice_engine) {
            TypioVoiceProxy *proxy =
                (TypioVoiceProxy *)svc->voice_engine->user_data;
            if (proxy) {
                typio_voice_proxy_clear_impl(proxy);
                typio_log(TYPIO_LOG_INFO,
                          "Voice model unloaded after %u ms idle",
                          svc->idle_timeout_ms);
            }
        }
...
static bool voice_idle_start_thread(TypioVoiceService *svc) {
    if (svc->idle_timer_fd < 0 || svc->idle_shutdown_efd < 0)
        return false;

    if (pthread_create(&svc->idle_thread, nullptr, voice_idle_thread, svc) != 0) {
        typio_log(TYPIO_LOG_ERROR, "Failed to create voice idle thread");
        return false;
    }
    return true;
}

static void voice_idle_stop_thread(TypioVoiceService *svc) {
    uint64_t val = 1;
    if (svc->idle_shutdown_efd >= 0)
        write(svc->idle_shutdown_efd, &val, sizeof(val));
    pthread_join(svc->idle_thread, nullptr);
}

static void voice_idle_timer_arm(TypioVoiceService *svc) {
    if (svc->idle_timer_fd < 0 || svc->idle_timeout_ms == 0)
        return;
    pthread_mutex_lock(&svc->buffer_mutex);
    svc->idle_armed = true;
    pthread_mutex_unlock(&svc->buffer_mutex);
    struct itimerspec its = {
        .it_value.tv_sec = svc->idle_timeout_ms / 1000,
        .it_value.tv_nsec = (svc->idle_timeout_ms % 1000) * 1000000L,
    };
    timerfd_settime(svc->idle_timer_fd, 0, &its, nullptr);
}

static void voice_idle_timer_disarm(TypioVoiceService *svc) {
    if (svc->idle_timer_fd < 0)
        return;
    pthread_mutex_lock(&svc->buffer_mutex);
    svc->idle_armed = false;
    pthread_mutex_unlock(&svc->buffer_mutex);
...
TypioVoiceService *typio_voice_service_new(TypioInstance *instance) {
    ...
    /* Create idle timer infrastructure */
    svc->idle_timer_fd = timerfd_create(CLOCK_MONOTONIC, TFD_NONBLOCK | TFD_CLOEXEC);
    svc->idle_shutdown_efd = eventfd(0, EFD_CLOEXEC | EFD_NONBLOCK);
    if (svc->idle_timer_fd >= 0 && svc->idle_shutdown_efd >= 0) {
        if (!voice_idle_start_thread(svc)) {
            close(svc->idle_timer_fd);
            close(svc->idle_shutdown_efd);
            svc->idle_timer_fd = -1;
            svc->idle_shutdown_efd = -1;
        }
    } else {
...
bool typio_voice_service_start(TypioVoiceService *svc) {
    ...
    /* Disarm idle timer before any voice activity */
    voice_idle_timer_disarm(svc);

    /* If model was unloaded, try to reload synchronously.
     * We must drop buffer_mutex because focus_in may touch proxy state. */
    pthread_mutex_lock(&svc->buffer_mutex);
    if (svc->voice_engine && svc->voice_engine->base_ops &&
        svc->voice_engine->base_ops->focus_in &&
        !engine_has_voice(svc->voice_engine)) {
        pthread_mutex_unlock(&svc->buffer_mutex);
        svc->voice_engine->base_ops->focus_in(svc->voice_engine, nullptr);
        pthread_mutex_lock(&svc->buffer_mutex);
    }
...
void typio_voice_service_stop(TypioVoiceService *svc) {
    ...
    if (pthread_create(&svc->infer_thread, nullptr, inference_thread, svc) != 0) {
        ...
        svc->audio_cap = 0;
        pthread_mutex_unlock(&svc->buffer_mutex);
        /* Arm idle timer since we are back to IDLE */
        voice_idle_timer_arm(svc);
        return;
    }
}
...
void typio_voice_service_dispatch(TypioVoiceService *svc,
    ...
    /* Start idle timer now that inference is done */
    voice_idle_timer_arm(svc);
...
void typio_voice_service_set_idle_timeout_ms(TypioVoiceService *svc,
    ...
    if (!svc)
        return;
    svc->idle_timeout_ms = timeout_ms;
}
