...
static DBusMessage *service_reply_from_json(DBusMessage *msg,
                                               const char *json) {
    ...
    err = strstr(json, "\"error\"");
    if (err) {
        const char *msg_start = strchr(err, ':');
        if (msg_start) {
            msg_start++;
            ...
            if (*msg_start == '\"') {
                ...
                if (msg_end) {
                    ...
                    if (buf) {
                        memcpy(buf, msg_start, len);
                        buf[len] = '\0';
                        DBusMessage *reply = dbus_message_new_error(msg, DBUS_ERROR_FAILED, buf);
                        free(buf);
                        return reply;
                    }
                }
        ...
        return dbus_message_new_error(msg, DBUS_ERROR_FAILED, "Service error");
    }
...
static DBusMessage *status_handle_activate_engine(TypioStatusBus *bus,
    ...
    snprintf(params, sizeof(params), "{\"engine\":\"%s\"}", engine_name);
    json = typio_status_service_handle(bus->service, "ActivateEngine", params, 0);
    reply = service_reply_from_json(msg, json);
    free(json);
    return reply;
}
...
static DBusMessage *status_handle_next_engine(TypioStatusBus *bus,
                                              DBusMessage *msg) {
    char *json = typio_status_service_handle(bus->service, "NextEngine", nullptr, 0);
    DBusMessage *reply = service_reply_from_json(msg, json);
    free(json);
    return reply;
}
...
static DBusMessage *status_handle_reload_config(TypioStatusBus *bus,
                                                DBusMessage *msg) {
    char *json = typio_status_service_handle(bus->service, "ReloadConfig", nullptr, 0);
    DBusMessage *reply = service_reply_from_json(msg, json);
    free(json);
    return reply;
}
...
static DBusMessage *status_handle_stop(TypioStatusBus *bus,
                                       DBusMessage *msg) {
    char *json = typio_status_service_handle(bus->service, "Stop", nullptr, 0);
    DBusMessage *reply = service_reply_from_json(msg, json);
    free(json);
    return reply;
}
...
static DBusMessage *status_handle_set_config_text(TypioStatusBus *bus,
    ...
    params = calloc(strlen(config_text) + 32, 1);
    if (!params) {
        return dbus_message_new_error(msg, DBUS_ERROR_NO_MEMORY, "OOM");
    }
    snprintf(params, strlen(config_text) + 32, "{\"content\":\"%s\"}", config_text);
    json = typio_status_service_handle(bus->service, "SetConfigText", params, 0);
    reply = service_reply_from_json(msg, json);
    free(json);
    free(params);
    return reply;
...
static DBusMessage *status_handle_set_rime_schema(TypioStatusBus *bus,
    ...
    params = calloc(strlen(schema) + 32, 1);
    if (!params) {
        return dbus_message_new_error(msg, DBUS_ERROR_NO_MEMORY, "OOM");
    }
    snprintf(params, strlen(schema) + 32, "{\"schema\":\"%s\"}", schema);
    json = typio_status_service_handle(bus->service, "SetRimeSchema", params, 0);
    reply = service_reply_from_json(msg, json);
    free(json);
    free(params);
    return reply;
...
static DBusMessage *status_handle_deploy_rime_config(TypioStatusBus *bus,
                                                     DBusMessage *msg) {
    char *json = typio_status_service_handle(bus->service, "DeployRimeConfig", nullptr, 0);
    DBusMessage *reply = service_reply_from_json(msg, json);
    free(json);
    return reply;
}
...
TypioStatusBus *typio_status_bus_new(TypioInstance *instance) {
    ...
    if (!bus->service) {
        typio_log(TYPIO_LOG_WARNING, "Failed to create TypioStatusService for D-Bus adapter");
        free(bus);
        return nullptr;
    }
