...
static bool mozc_sync_output(const mozc::commands::Output &output,
    ...
    comp.struct_size = sizeof(TypioComposition);
    comp.selected = -1;
    TypioPreeditSegment *segments = nullptr;
    TypioCandidate *items = nullptr;
    char **labels = nullptr;
    int count = 0;
    ...
    if (output.has_preedit() && output.preedit().segment_size() > 0) {
        ...
        segments = static_cast<TypioPreeditSegment *>(
        ...
        if (segments) {
            ...
            comp.segments = segments;
            comp.segment_count = static_cast<size_t>(seg_count);
            comp.cursor_pos = preedit.has_cursor() ? static_cast<int>(preedit.cursor()) : 0;
            has_preedit = true;
        }
    }
    ...
    if (output.has_candidate_window() && output.candidate_window().candidate_size() > 0) {
        ...
        count = cands.candidate_size();

        items = static_cast<TypioCandidate *>(
            calloc(static_cast<size_t>(count), sizeof(TypioCandidate)));
        labels = static_cast<char **>(
        ...
        if (items && labels) {
            ...
            comp.candidate_count = static_cast<size_t>(count);
            comp.page = page_size > 0 ? first_index / page_size : 0;
            comp.page_size = page_size;
            comp.total = total;
            comp.selected = focused >= 0 ? focused - first_index : -1;
            comp.has_prev = first_index > 0;
            comp.has_next = first_index + count < total;
            comp.content_signature = 0;
            has_candidates = true;
        }
    }

    /* Emit one atomic composition */
