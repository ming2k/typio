...
- selector widgets should prefer the shared `ControlStateBinding` model over bespoke sync logic
...
Both the tray menu and control center use `typio_rime_schema_list_load()` from the core library (`typio/rime_schema_list.h`). This ensures schema lists are always consistent regardless of which surface the user is looking at. Neither surface should implement its own schema file parsing. In `typio-control`, schema option refresh should flow through the selector binding layer instead of direct one-off widget code.
